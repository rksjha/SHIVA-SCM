...
const INITIAL_FORM = {
  ...
  gstin: '', pan_number: '', supplier_type: 'primary',
  credit_limit: '', credit_period_days: '30', payment_terms: 'Net 30',
  status: 'active', notes: '',
};
...
export default function Suppliers() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await supplierAPI.getAll({ page, limit: 10, search, status: statusFilter });
      setData(res.data.suppliers || res.data.data || []);
  ...
  const handleSave = async (e) => {
    ...
    try {
      editItem ? await supplierAPI.update(editItem.id, form) : await supplierAPI.create(form);
  ...
  const handleDelete = async () => {
    try { await supplierAPI.delete(deleteItem.id); toast.success('Deleted'); setShowDelete(false); load(); }
  ...
  const columns = [
    { key: 'supplier_id', label: 'SUP ID', render: (v) => <span className="entity-id">{v}</span> },
    { key: 'company_name', label: 'Company' },
    { key: 'contact_person', label: 'Contact' },
    { key: 'city', label: 'City' },
    { key: 'phone', label: 'Phone' },
    { key: 'supplier_type', label: 'Type', render: (v) => <Badge status={v} label={v} /> },
    { key: 'credit_limit', label: 'Credit Limit', render: (v) => v ? `₹${Number(v).toLocaleString('en-IN')}` : '—' },
  ...
  return (
    <Layout>
      <div className="page-header">
        <div>
          <h1 className="page-title"><Truck size={22} /> Suppliers</h1>
          <p className="page-subtitle">Manage your supplier network and procurement sources</p>
        </div>
      ...
      <div className="card">
        <div className="filter-bar">
          <input className="form-control" placeholder="Search supplier…" value={search}
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => downloadCSV(data, 'suppliers.csv')}>Export CSV</button>
        </div>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)}
        title={editItem ? 'Edit Supplier' : 'Add New Supplier'} size="lg"
        footer={
        ...
        <form onSubmit={handleSave}>
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={form.supplier_type} onChange={f('supplier_type')}>
      ...
      <Modal isOpen={showView} onClose={() => setShowView(false)} title="Supplier Details" size="lg">
        {viewItem && (
          <div className="detail-grid">
            <div className="detail-header">
              <span className="entity-id large">{viewItem.supplier_id}</span>
              <Badge status={viewItem.status} label={viewItem.status} />
              <Badge status={viewItem.supplier_type} label={viewItem.supplier_type} />
            </div>
            ...
            <div className="detail-section">
              ...
              <div className="detail-row"><span>Credit Limit</span><strong>{viewItem.credit_limit ? `₹${Number(viewItem.credit_limit).toLocaleString('en-IN')}` : '—'}</strong></div>
